...
window.editProduct = async function(id){
    const item = products.find(p=>p.id===id);
    const newName = prompt("Nombre:", item.name); if(!newName) return;
    const newDesc = prompt("Descripción:", item.desc); if(!newDesc) return;
    const newPrice = parseFloat(prompt("Precio:", item.price)); if(isNaN(newPrice)) return;
